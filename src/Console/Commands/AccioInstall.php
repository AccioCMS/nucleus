...
use Accio\Theme\Services\DummyTheme;
use Accio\Console\SystemRequirements;
use Accio\Support\Environment;
use Accio\Console\AccioInstallOptions;
...
class AccioInstall extends Command
{
    ...
    private function askInstallingQuestions()
    {
        ...
        // Timezone
        $regions = AccioInstallOptions::getTimezoneRegions();
        $this->TIMEZONE = $this->choice('Timezone region', array_keys($regions), 0);
        if ($this->TIMEZONE !== 'UTC') {
            $locations = AccioInstallOptions::getTimezoneLocations($regions[$this->TIMEZONE]);
