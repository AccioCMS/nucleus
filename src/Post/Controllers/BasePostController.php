...
namespace Accio\Post\Controllers;
...
class BasePostController extends MainController
{
    ...
    public function getAllPosts($lang, $postType)
    {
        ...
        try{
            $orderColumnType = DB::connection()
                ->getDoctrineColumn($postType, $orderBy)
                ->getType()
                ->getName();
        }catch (\Exception $e){
    ...
    {
        Event::listen(
            'post:table_list_rows', function ($results) use ($postType) {
                $rows = [];
    ...
    private function getPostRelatedMedia($id, $post, $postTypeSlug, $translatableFields)
    {
        ...
        $mediaRelationsResults = DB::table($mediaRelationTable)
            ...
            ->select(
                'media.title as title', 'media.mediaID', $mediaRelationTable.'.mediaRelationID',
