...
if (! function_exists('googleTagManager')) {
    /**
     * Get google tag manager head script
     *
...
if (! function_exists('metaTags')) {
     ...
     * @throws Exception
     */
    function metaTags($model = null, $customData = [])
    {
        ...
        if(!$model) {
            $model = $currentMenuLink;
        }

        // Set model data
        if($model) {
            \Accio\Support\Facades\Meta::setModelData($model);
        ...
        }elseif($model && property_exists($model, 'title')) {
            $title = $model->title;
        ...
        }elseif($model && property_exists($model, 'description')) {
        ...
        event('meta:add', [$model, $customData]);
...
if (! function_exists('isInAdmin')) {
    /**
     * Check if current request belong to admin.
     *
     * @return bool
     */
    function isInAdmin()
